...
// ÔÚcode_book[]ÖÐÕÒµ½Óëfreq×î½Ó½üµÄÖµ£¬·µ»ØÏÂ±ê
...
size_t update_code_idx(double freqs[], size_t num, double code_book[],
  ...
  size_t changed = 0;
  // ½«ÏµÍ³´ÊÆµ¹éÂ£³É256¸öÖµ£¬¾ßÌå×ö·¨ÊÇ£º
  // 1¡¢ÔÚÏµÍ³´Ê¿âÖÐ´ÓÍ·ÍùºóÕÒ£¬ÕÒ³ö256¸ö²»Í¬µÄ´ÊÆµ£»
  // 2¡¢±éÀúÏµÍ³´Ê¿â£¬¶ÔÓÚÃ¿Ò»¸ö´ÊÆµ£¬´ÓÕâ256¸öÖÐÕÒµ½ÓëÖ®×î½Ó½üµÄ£¬×÷Îª¸Ã´ÊµÄ´ÊÆµ
  // ½«×î½Ó½üµÄÔªËØÏÂ±ê¼ÇÂ¼µ½code_idxÖÐ
  for (size_t pos = 0; pos < num; pos++) {
    CODEBOOK_TYPE idx;
    // ÔÚcood_bookÖÐÕÒµ½Óëfreqs[pos]×î½Ó½üµÄÖµ£¬·µ»ØÆäÏÂ±ê
...
double recalculate_kernel(double freqs[], size_t num, double code_book[],
  ...
  for (size_t pos = 0; pos < num; pos++) {
    ...
    cb_new[code_idx[pos]] += freqs[pos];  // ËùÓÐÖ¸Ïò¸ÃÎ»ÖÃµÄfreqs×ÜºÍ
    item_num[code_idx[pos]] += 1;         // ËùÓÐÖ¸Ïò¸ÃÎ»ÖÃµÄfreqsµÄÔªËØ¸öÊý
  }

  for (size_t code = 0; code < kCodeBookSize; code++) {
    assert(item_num[code] > 0);
    code_book[code] = cb_new[code] / item_num[code]; // ËùÓÐÖ¸Ïò¸ÃÎ»ÖÃµÄfreqsµÄ¾ùÖµ
  }

  delete [] item_num;
  delete [] cb_new;

  return ret; // ·µ»Ø¡°lemma_arrÖÐ´ÊÆµºÍ¹éÂ£´ÊÆµµÄ²î¡±µÄ×ÜºÍ
}
...
void iterate_codes(double freqs[], size_t num, double code_book[],
  ...
  double delta_last = 0;
  do {
    // ½«ÏµÍ³´Ê¿âÖÐÃ¿¸ö´ÊµÄ´ÊÆµ¹éÂ£³É256¸öÖµ£¬¶ÔÓÚlemma_arrÖÐµÄÃ¿¸öÔªËØi£¬
    // code_book[ code_idx[i] ] ÊÇÓë¸Ã´Ê´ÊÆµ×î½Ó½üµÄ¹éÂ£ºóµÄ´ÊÆµ
    size_t changed = update_code_idx(freqs, num, code_book, code_idx);

    // ¶ÔÓÚcode_bookÖÐÃ¿¸öÔªËØ£¬È¡ÔÚlemma_arr¹éÂ£µ½¸ÃÔªËØµÄËùÓÐ´ÊÌõµÄfreqÆ½¾ùÖµ£¬ÖØÐÂ¸³¸ø¸ÃÔªËØ
    // delatÊÇ ¦²(lemma_arrÃ¿¸ö´ÊÆµ-Æä¹éÂ£Öµ)
...
bool NGram::build_unigram(LemmaEntry *lemma_arr, size_t lemma_num,
  ...
  double max_freq = 0;
  idx_num_ = idx_now + 1; // lemma_arr_ÏµÍ³´ÊÌõ¸öÊý+1
  assert(idx_now + 1 == next_idx_unused);

  // 归一化freqs，其idx是在lemma_arr中的索引
  for (size_t pos = 0; pos < idx_num_; pos++) {
    freqs[pos] = freqs[pos] / total_freq; // ÕÛËã·½·¨
    assert(freqs[pos] > 0);
  ...
  size_t freq_pos = 0;
  // ÔÚÏµÍ³´ÊÆµÀï´ÓÍ·ÍùºóÕÒ£¬ÕÒµ½Ç°256¸ö²»Í¬µÄ´ÊÆµ·Åµ½freq_codes_df_[256]ÖÐ
  ...
  // ½«ÏµÍ³´Ê¿âµÄ´ÊÆµ¹éÂ£µ½freq_codes_df_ÖÐµÄ256¸öÖµÉÏÈ¥£¬²¢¾­¹ý¶àÂ×µü´ú£¬ÈÃ
  // ¦²(ÏµÍ³´ÊÌõµÄ´ÊÆµ - Æä¹éÂ£Öµ)×îÐ¡
