...
bool SpellingTrie::build_ym_info() {
  ...
  // ½«ÔÏÄ¸Ìí¼Óµ½¹þÏ£±íspl_table->raw_spellings_ÖÐ
  ...
  size_t ym_num;
  const char* ym_buf;
  ym_buf = spl_table->arrange(&ym_item_size, &ym_num); // °´ÔÏÄ¸×Ö´®ÅÅÐò
  ...
  memcpy(ym_buf_, ym_buf, sizeof(char) * ym_item_size * ym_num); // ½«ÔÏÄ¸¿½±´µ½ym_buf_ÖÐ
