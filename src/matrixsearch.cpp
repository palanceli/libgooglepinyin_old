...
#define PRUMING_SCORE 8000.0
    ...
    bool MatrixSearch::add_char(char ch) {
        if (!prepare_add_char(ch)) // 往pys_追加拼音，往matrix_追加一个数据块
            return false;
    ...
    size_t MatrixSearch::extend_mtrx_nd(MatrixNode *mtrx_nd, LmaPsbItem lpi_items[],
        ...
        LOGBEGIN("更新mtrx_nd_pool_");
        printf("id    score from              dmi_fr step         hz py     \n");
        for(auto mtrx_pos=0; mtrx_pos<=pys_decoded_len_; mtrx_pos++){
            for(auto mtrx_nd_pos=0; mtrx_nd_pos<matrix_[mtrx_pos].mtrx_nd_num; mtrx_nd_pos++){
                MatrixNode *mtrx_nd = mtrx_nd_pool_ + matrix_[mtrx_pos].mtrx_nd_pos + mtrx_nd_pos;
                printf("%-5lu %-5.0f", mtrx_nd->id, mtrx_nd->score);
                if(mtrx_nd->from == NULL){
                    printf(" NULL             ");
                }else{
                    auto idx = mtrx_nd->from - mtrx_nd_pool_;
                    printf(" &mtrx_nd_pool_[%-d]", idx);
                }
                printf(" %-6d %-4d", mtrx_nd->dmi_fr, mtrx_nd->step);
                char16 str[kMaxLemmaSize] = {0};
                uint16 str_len = get_lemma_str(mtrx_nd->id, str, kMaxLemmaSize + 1);
                std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t> convert;
                std::string strTemp = convert.to_bytes((char16_t*)str);
                printf(" %10s", strTemp.c_str());
                uint16 splids[32] = {0};
                str_len = dict_trie_->get_lemma_splids(mtrx_nd->id, splids, 32, false);
                strTemp = "";
                for(auto i=0; i<str_len; i++){
                    strTemp += ((SpellingTrie*)spl_trie_)->get_spelling_str(splids[i]);
                    strTemp += " ";
