...
void DictBuilder::get_top_lemmas() { // ȡ����Ƶ����top_lmas_num_�����ɴ�С�ŵ�top_lmas_��
  top_lmas_num_ = 0;
  if (NULL == lemma_arr_)
    return;
...
size_t DictBuilder::read_raw_dict(const char* fn_raw,
  ...
  // 读取文件 rawdict_utf16_65105_freq.txt
...
bool DictBuilder::build_dict(const char *fn_raw,
  ...
  // ��lma_nodes_le0������dict_trie->root_����lma_node_ge1_������dict_trie->nodes_ge1_
...
size_t DictBuilder::build_scis() {
  ...
  for (size_t pos = 0; pos < lemma_num_; pos++) {
    ...
    for (size_t hzpos = 0; hzpos < hz_num; hzpos++) {
      ...
      if (1 == hz_num)
        scis_[scis_num_].freq = lemma_arr_[pos].freq; // �ڴʿ�����ǵ��ֵģ���Ƶ=��Ƶ
      else
        scis_[scis_num_].freq = 0.000001;             // ���������Сֵ
      scis_num_++;
    }
  }

  myqsort(scis_, scis_num_, sizeof(SingleCharItem), cmp_scis_hz_splid_freq); // ���պ��������������ͬ���������Ҳ��ͬ����Ƶ
...
bool DictBuilder::construct_subset(void* parent, LemmaEntry* lemma_arr,
  ...
  // ��ǰ��һ��(level��)�ж���Ԫ�أ���¼��parent_son_num
  ...
  if (parent_son_num > max_sonbuf_len_[level])
    max_sonbuf_len_[level] = parent_son_num;  // ��¼�ò�Ԫ�ظ���
  ...
  for (size_t i = item_start + 1; i < item_end; i++) {
    ...
    if (spl_idx_current == spl_idx_node) {
      ...
      if (i - item_start_next > homo_num) { // ˵���ڵ�ǰ����»����ӽڵ�
        void *next_parent;
        if (0 == level)
